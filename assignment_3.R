...
# initial portfolio
original.portfolio <- oil.lookalikes(text.source = section.1.business,
                                     bound.lower = 5,
                                     bound.upper = 50,
                                     n.firms = 25)
...
compare.portfolios <- function(var.prefix, df) {
  temp.df <- raw.data %>%
    ...
    mutate(portfolio = ifelse(industry.fama.french.49 == "30 Oil", "oil", "unknown"),
           portfolio = factor(ifelse(cik %in% df$cik, "oil.tracking", portfolio))) %>% 
    ...
    select(-portfolio) %>% 
    t() %>%
    `colnames<-`(c("oil", "oil.tracking", "unknown")) %>% 
    as.data.frame() %>% 
    select(-unknown)
...
final.df %>% 
  ggplot(aes(x = months)) + 
  geom_line(aes(y = oil * 100, color = "Oil")) +
  geom_line(aes(y = oil.tracking * 100, color = "Oil tracking, initial")) +
  geom_line(aes(y = oil.tracking1 * 100, color = "Min 50, max 100 doc")) +
  geom_line(aes(y = oil.tracking2 * 100, color = "More than 100 doc")) + 
  geom_line(aes(y = oil.tracking3 * 100, color = "Section 7 MDA")) +
  geom_line(aes(y = oil.tracking4 * 100, color = "50 firms")) +
  geom_line(aes(y = oil.tracking5 * 100, color = "100 firms")) +
...
# clean df
final.df %<>% 
  select(-c(oil1, oil2, oil3, oil4, oil5)) %>% 
  rename(min50.max100.doc.return = oil.tracking1,
         more.than.100.doc.return = oil.tracking2,
         section.7.mda.return = oil.tracking3,
         fifty.firms.return = oil.tracking4,
         hundred.firms.return = oil.tracking5,
         oil.tracking.return = oil.tracking,
         oil.tracking.diff = original.portfolio.diff,
         oil.tracking.rmse = original.portfolio.rmse,
         oil.tracking.mae = original.portfolio.mae,
         oil.tracking.cor = original.portfolio.cor,
         oil.return = oil)